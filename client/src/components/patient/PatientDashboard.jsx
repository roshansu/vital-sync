    ...
    <div className="min-h-screen ml-0 font-sans" style={{ backgroundColor: colors.surface, color: colors.onSurface }}>
      <main className="p-4 md:p-8 max-w-7xl mx-auto">
        ...
        <section className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-10">
          <StatCard icon="event" label="Upcoming Appointments" value={data?.upcomingAppointment || '0'} bgColor={colors.primaryFixed} iconColor={colors.primary} />
          <StatCard icon="folder_shared" label="Total Visits" value={data?.totalVisit || '0'} bgColor={colors.secondaryFixed} iconColor={colors.secondary} />
          <StatCard icon="payments" label="Pending Bills" value={data?.pendingBill || '0'} bgColor={colors.tertiaryFixed} iconColor={colors.tertiary} />
        </section>
              ...
              <div className="relative z-10">
                <div className="flex justify-between items-start mb-10">
                  <div>
                    ...
                    <h4 className="text-3xl font-black mt-4 leading-tight">{data?.nextAppointment[0]?.name}</h4>
                    <p className="font-medium tracking-wide opacity-80">{data?.nextAppointment[0]?.specialization}</p>
                  </div>
                  <div className="w-16 h-16 rounded-full border-4 border-white/10 overflow-hidden">
                    <img 
                      className="w-full h-full object-cover" 
                      src={data?.nextAppointment[0]?.imageUrl || "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcStvL8kNVKvJskGpi8do02RNw2bn3sKxTTJ2g&s"} 
                      alt="Doctor" 
                    />
                  </div>
                </div>
                ...
                <div className="flex flex-wrap gap-8 items-center border-t border-white/10 pt-8">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="font-bold">{data?.nextAppointment[0]?.date}</p>
                    </div>
                  </div>
                  <div className="flex items-center gap-3">
                    ...
                    <p className="font-bold">{data?.nextAppointment[0]?.time}</p>
                    </div>
                  </div>
            ...
            <div className="rounded-2xl overflow-hidden shadow-sm" style={{ backgroundColor: colors.surfaceContainerLowest }}>
              <div className="p-6 space-y-6">
                <ActivityItem 
                  ...
                  title={data?.recentActivity?.prescription[0]?.title || ""} 
                  subtitle={data?.recentActivity?.prescription[0]?.subTitle || ""}
                  date={data?.recentActivity?.prescription[0]?.date || ""}
                />
                <ActivityItem 
                  dotColor={colors.tertiary} 
                  label="Appointment Request" 
                  title={data?.nextAppointment[0]?.name} 
                  subtitle={data?.nextAppointment[0]?.specialization} 
                  date={data?.nextAppointment[0]?.date}
                  isLast 
                />
              </div>
            </div>
